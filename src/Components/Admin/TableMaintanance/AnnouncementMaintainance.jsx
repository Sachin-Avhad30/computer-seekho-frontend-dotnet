...
const AnnouncementManagement = () => {
  ...
  const fetchAnnouncements = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5087/api/announcements",
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(
          `http://localhost:5087/api/announcements/${editingId}`,
          formData,
        );
      } else {
        await axios.post("http://localhost:5087/api/announcements", formData);
      }
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this announcement?")) {
      try {
        await axios.delete(`http://localhost:5087/api/announcements/${id}`);
        fetchAnnouncements();
      } catch (error) {
