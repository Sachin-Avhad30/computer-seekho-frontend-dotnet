...
const RegisteredStudentsPage = () => {
  ...
  const fetchRegisteredStudents = async () => {
    ...
    try {
      const data = await getAllRegisteredStudents();
      console.log("✅ Registered students from API:", data);
  ...
  const applyFilters = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (student) =>
          ...
          String(student.studentMobile || "").includes(searchTerm), // ✅ Converts to string first,
      );
    }
    ...
    if (filterCourse) {
      filtered = filtered.filter(
        (student) => student.courseId === parseInt(filterCourse),
      );
    }
    ...
    if (filterBatch) {
      filtered = filtered.filter(
        (student) => student.batchId === parseInt(filterBatch),
      );
    }
